...
import type { ConnectorStatus, SyncJob } from '@/shared/types'

import { ConnectDialog } from './connect-dialog'
import { MarketplaceLogo } from './marketplace-brand'
import { useConnectorRealtimeEvent } from '../model/realtime'
...
export function ConnectorCard({ connector, readOnly = false, dict, lang }: Props) {
  ...
  const [liveJob, setLiveJob] = useState<SyncJob | null>(null)
  ...
  useConnectorRealtimeEvent((event) => {
    ...
    const job = event.payload as unknown as SyncJob
    if (job.connector_name === safeName && typeof job.job_id === 'string') {
      setLiveJob(job)
    }
  })
          ...
          {effectiveJob && (
            <div className="space-y-1.5 rounded-md border border-border bg-muted/40 px-3 py-2 text-xs">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={JOB_STATUS_VARIANT[effectiveJob.status] ?? 'secondary'}
                  className="text-[10px]"
                >
                  {jobStatusLabels[effectiveJob.status] ?? effectiveJob.status}
                </Badge>
              </div>
              <div className="grid grid-cols-3 gap-1 text-muted-foreground">
                <span>{formatMessage(dict.connectors.card.orders, { count: effectiveJob.orders_synced ?? 0 })}</span>
                <span>{formatMessage(dict.connectors.card.payments, { count: effectiveJob.payments_synced ?? 0 })}</span>
                <span>{formatMessage(dict.connectors.card.fees, { count: effectiveJob.fees_synced ?? 0 })}</span>
              </div>
              {effectiveJob.error_message && (
                <p className="text-destructive">{effectiveJob.error_message}</p>
              )}
            </div>
          )}
          ...
          {isConnected ? (
            <form action={syncFormAction}>
              ...
              <Button
                ...
                variant="outline"
                className="w-full"
                disabled={readOnly || isSyncing || jobIsRunning}
              >
                {isSyncing || jobIsRunning ? (
                  <>
