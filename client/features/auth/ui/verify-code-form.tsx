    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-xl border border-border bg-muted/40 p-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Digite o codigo de verificacao enviado para este e-mail para liberar o acesso ao Brasaller.
        </p>
      </div>
      ...
      <form action={verifyAction} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          />
          <p className="text-xs text-muted-foreground">Use o codigo de 6 digitos enviado para o seu e-mail.</p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-background p-4">
          <div className="flex items-start gap-3 text-sm text-muted-foreground">
            ...
            <p className="leading-6">
              Sua conta permanece bloqueada ate a validacao do e-mail. Nenhum token de sessao e criado antes dessa etapa.
            </p>
          </div>
        </div>

        <Button type="submit" size="lg" className="w-full" disabled={isVerifying}>
          {isVerifying ? <><Loader2 className="animate-spin" />Validando codigo...</> : 'Validar e continuar'}
        </Button>
      </form>
      ...
      <form action={resendAction}>
        ...
        <Button type="submit" variant="outline" className="w-full" disabled={isResending}>
          {isResending ? <><Loader2 className="animate-spin" />Reenviando codigo...</> : <><RotateCw />Reenviar codigo</>}
        </Button>
      </form>
    </div>
  )
