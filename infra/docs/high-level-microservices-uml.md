...
- MVP sem broker: comunicacao interna por REST, jobs com Quarkus Scheduler e controle de retry/idempotencia em tabelas PostgreSQL quando necessario.
...
flowchart LR
    ...
    subgraph Processing["Processamento interno MVP"]
        QuarkusScheduler["Quarkus Scheduler<br/>jobs recorrentes"]
        InternalRest["REST interno<br/>X-Internal-Token"]
        JobTables["PostgreSQL job/outbox tables<br/>retry e idempotencia"]
    end
    ...
    subgraph External["Integracoes externas"]
        ...
        Clicksign["Clicksign API 3.0 Envelope<br/>assinatura balanco/DRE"]
        SMTP["SMTP/Mailer"]
    end
    ...
    Core -->|"sync-all e status"| JobTables
    Core -->|"POST /reports/internal/entries"| InternalRest
    InternalRest --> Reporting
    Core -->|"POST /notifications/events/new-sale"| InternalRest
    InternalRest --> Notification
    Reporting -->|"calculo DRE sob demanda/job"| JobTables
    QuarkusScheduler -->|"processa jobs pendentes"| Core
    QuarkusScheduler -->|"processa alertas e fechamentos"| Notification
    QuarkusScheduler -->|"processa DRE/fechamento"| Reporting
    ...
    Reporting -. "balanco/DRE para assinatura<br/>webhook de conclusao" .-> Clicksign
...
| `core-service` | Contexto tenant-aware e contratos de marketplace | Valida contexto JWT, aplica leitura/escrita por papel, resolve conector por nome, processa `sync-all`, normaliza pedidos/pagamentos/taxas/notas e chama contratos internos de Reporting e Notification. Tokens de marketplace ficam criptografados e nunca retornam ao frontend. | REST interno com `X-Internal-Token`, adapters `MarketplaceConnector`; hoje existem `sandbox` e `mercado-livre`; jobs Quarkus quando necessario. | `tenant_context_audit`, `marketplace_connector_tokens` com AES-256, `connector_sync_jobs`. |
| `notification-service` | Notificacoes, alertas, e-mail e analytics de venda | Gerencia preferencias por tenant, cria notificacoes in-app, envia e-mail quando habilitado, recebe eventos internos por REST e executa jobs automaticos de fechamento mensal, liberacao ML e relatorio semanal. | SMTP/Mailer, `reporting-service` por REST interno, JWT HS256, `X-Internal-Token` para eventos internos, Quarkus Scheduler. | `notification_preferences`, `notifications`, `notification_deliveries`, tabelas de idempotencia quando necessario. |
...
| `reporting-service` | Painel financeiro, relatorios, fiscal MVP e exportacoes | Materializa lancamentos por tenant via endpoint interno, soma faturado/recebido/taxas/a receber, oferece filtros, busca, ordenacao, tabela, graficos, exportacao PDF/XLSX/CSV, perfil fiscal, assinatura de upload Cloudinary, despesas com comprovante obrigatorio, DRE simplificada, jobs internos de DRE, fechamento mensal assinado e consultas internas para automacoes. | REST interno com `X-Internal-Token`, JWT HS256 para leitura; escrita fiscal por `ADMIN`/`VENDEDOR`; `CONTADOR` read-only e assinante do fechamento; renderizadores `ReportExportRenderer`; Quarkus Scheduler. | `report_entries`, `tenant_fiscal_profiles`, `expense_entries`, `dre_calculation_jobs`, `accounting_period_closings`. |
...
flowchart TB
    subgraph Pattern["Padrao interno por microservice"]
        ...
        Infra["infrastructure<br/>JDBC, REST client, JWT, Scheduler, Mailer, Keycloak"]
...
classDiagram
    ...
    ConnectorService ..> NotificationService : envia evento interno por REST
...
## Fluxo UML - Sincronizacao com REST Interno, Reporting e Notificacao
...
sequenceDiagram
    ...
    participant CoreDB as core_service DB
    ...
    Core->>CoreDB: cria connector_sync_job QUEUED
    Core-->>Client: 202 Accepted { job_id, status=QUEUED }

    Core->>Core: Quarkus Scheduler processa job pendente
    Core->>CoreDB: status PROCESSING
    Core->>Connector: syncAll(tenantId, since)
    Connector-->>Core: SyncResult
    Core->>Connector: getOrders(from=since)
    loop para cada pedido sincronizado
    ...
    loop para cada pedido pago
        Core->>Notification: POST /notifications/events/new-sale<br/>X-Internal-Token
    end
    Core->>CoreDB: status DONE/FAILED
    ...
    alt newSaleEnabled = true
        ...
        Notification-->>Core: evento processado sem criar notificacao
    end
```
...
No MVP cloud, o projeto nao usa broker externo. Os fluxos internos sao coordenados por REST, Quarkus Scheduler e tabelas PostgreSQL.

| Fluxo | Produtor | Processador | Persistencia/controle |
| --- | --- | --- | --- |
| Sincronizacao de conector | `core-service` REST | `core-service` via Quarkus Scheduler | `connector_sync_jobs` |
| Upsert de lancamento financeiro | `core-service` | `reporting-service` via `/reports/internal/entries` | `report_entries` |
| Nova venda/notificacao | `core-service` | `notification-service` via `/notifications/events/new-sale` | `notifications`, `notification_deliveries` |
| Calculo de DRE | `reporting-service` REST | `reporting-service` via Quarkus Scheduler/job interno | `dre_calculation_jobs` |
| Jobs recorrentes | Quarkus Scheduler | `notification-service` e `reporting-service` | tabelas de dominio e logs |

Se o volume crescer, esses contratos internos podem evoluir para um broker gerenciado sem alterar o gateway publico.
...
- `reporting-service` possui read model financeiro alimentado por endpoint interno protegido, endpoints de dashboard/graficos/tabela, nucleo fiscal MVP com regime tributario, despesas com comprovante Cloudinary obrigatorio, DRE simplificada sob demanda, jobs internos de DRE e fechamento contabil assinado, alem de motor unico de exportacao PDF/XLSX/CSV.
...
- Eventos internos de notificacao existem por REST (`/notifications/events/**`), ficam bloqueados no gateway publico e sao chamados internamente com `X-Internal-Token`.
- O broker externo foi removido do MVP cloud. `core-service`, `notification-service` e `reporting-service` devem usar REST interno, Quarkus Scheduler e tabelas PostgreSQL para jobs/retry/idempotencia.
