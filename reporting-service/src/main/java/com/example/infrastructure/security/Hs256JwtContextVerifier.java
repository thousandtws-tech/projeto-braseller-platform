...
public class Hs256JwtContextVerifier implements AccessTokenVerifier {
    ...
    @ConfigProperty(name = "reporting.jwt.issuer")
    String issuer;

    @ConfigProperty(name = "reporting.jwt.audience")
    String audience;

    @ConfigProperty(name = "reporting.jwt.secret")
    String secret;
    ...
    public TenantContext verify(String authorizationHeader) {
        ...
        List<String> accountantTenantIds = listClaim(claims, "accountant_tenant_ids");
        boolean readOnly = roles.contains("CONTADOR") && !roles.contains("ADMIN");
        return new TenantContext(tenantId, userId, email, roles, readOnly, accountantTenantIds);
    }
